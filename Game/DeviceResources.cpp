...
DeviceResources::DeviceResources(DXGI_FORMAT backBufferFormat,
    ...
    : m_screenViewport{}, m_backBufferFormat(backBufferFormat),
      m_depthBufferFormat(depthBufferFormat), m_backBufferCount(backBufferCount),
      m_d3dMinFeatureLevel(minFeatureLevel), m_window(nullptr),
      m_d3dFeatureLevel(D3D_FEATURE_LEVEL_9_1), m_outputSize{0, 0, 1, 1},
      m_colorSpace(DXGI_COLOR_SPACE_RGB_FULL_G22_NONE_P709), m_options(flags | kFlipPresent),
...
void DeviceResources::CreateDeviceResources() {
    ...
    if (m_options & kEnableHDR) {
        ComPtr<IDXGIFactory5> factory5;
        if (FAILED(m_dxgiFactory.As(&factory5))) {
            m_options &= ~kEnableHDR;
            #ifdef _DEBUG
    ...
    if (m_options & kFlipPresent) {
        ComPtr<IDXGIFactory4> factory4;
        if (FAILED(m_dxgiFactory.As(&factory4))) {
            m_options &= ~kFlipPresent;
            #ifdef _DEBUG
...
void DeviceResources::CreateWindowSizeDependentResources() {
    ...
    const DXGI_FORMAT backBufferFormat = (m_options & (kFlipPresent | kAllowTearing | kEnableHDR))
    ...
    if (m_swapChain) {
        ...
        HRESULT hr = m_swapChain->ResizeBuffers(
            ...
            backBufferHeight,
            backBufferFormat,
            (m_options & kAllowTearing) ? DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING : 0u);
        ...
        swapChainDesc.SwapEffect = (m_options & (kFlipPresent | kAllowTearing | kEnableHDR))
                                       ? DXGI_SWAP_EFFECT_FLIP_DISCARD
                                       : DXGI_SWAP_EFFECT_DISCARD;
        swapChainDesc.AlphaMode = DXGI_ALPHA_MODE_IGNORE;
        swapChainDesc.Flags = (m_options & kAllowTearing) ? DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING : 0u;
    ...
    CD3D11_RENDER_TARGET_VIEW_DESC renderTargetViewDesc(D3D11_RTV_DIMENSION_TEXTURE2D,
                                                        m_backBufferFormat);
    ThrowIfFailed(
        m_d3dDevice->CreateRenderTargetView(m_renderTarget.Get(),
    ...
    if (m_depthBufferFormat != DXGI_FORMAT_UNKNOWN) {
        ...
        ThrowIfFailed(m_d3dDevice->CreateTexture2D(&depthStencilDesc,
                                                   nullptr,
...
void DeviceResources::UpdateColorSpace() {
    ...
    if ((m_options & kEnableHDR) && isDisplayHDR10) {
